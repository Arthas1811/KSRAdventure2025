...
public class Construction : MonoBehaviour
{
    ...
    public float HotspotSphereRadius = 70f;
    public float HotspotLongitudeScale = 2f;
    public float HotspotLatitudeOffset = 1f;
    public float PreviewRotationX = 40f;
    public float PreviewRotationY = 0f;
    public float CameraRotationClampAngle = 90f;
    public float ObjectRotationStep = 90f;
    public float MouseDragThreshold = 5f;
    public float ScreenCenterFactor = 0.5f;
    public float FOVStep = 2f;
    public float MinFOV = 5f;
    public float MaxFOV = 60f;
    public float StabilityWarningOffset = 0.6f;
    ...
    public CameraMovementConstruction CameraMovement;
    ...
    {   
        ...
        foreach (var customHotspot in customHotspots)
        {
            ...
            foreach (var coordinates in polygonCoordiantes)
            {
                ...
                float longitude = u * HotspotLongitudeScale * Mathf.PI;
                float latitude = (HotspotLatitudeOffset - v) * Mathf.PI;
                float x = HotspotSphereRadius * Mathf.Sin(latitude) * Mathf.Cos(longitude) * -1f;
                float y = HotspotSphereRadius * Mathf.Cos(latitude);
                float z = HotspotSphereRadius * Mathf.Sin(latitude) * Mathf.Sin(longitude);
    ...
    void CompleteAction(string action, string[] requirements, string currentImage, float zoomDuration, float FOV, Dictionary<GameObject, string[]> hotspotActions, Dictionary<GameObject, string[]> hotspotRequirements, GameObject hotspot = null)
    {
        ...
        else
        {
            ...
            if (activeState["x"] != null || activeState["y"] != null)
                CameraMovement.SetNewRotation(Mathf.Clamp(finalX, -CameraRotationClampAngle, CameraRotationClampAngle), finalY);
    ...
    void Start()
    {
        ...
        if (CameraMovement == null)
            CameraMovement = FindObjectOfType<CameraMovementConstruction>();

        if (AudioManagerConstruction.Instance == null)
        {
            AudioManagerConstruction existingAudioManager = FindObjectOfType<AudioManagerConstruction>();
            if (existingAudioManager != null)
                AudioManagerConstruction.Instance = existingAudioManager;
        }

        string locationName = "Polygons";
        string entryImage = null;
    ...
    void Update()
    {
        if (ShowInstructions == false)
        {
            ...
            {
                Vector2 screenCenter = new Vector2(Screen.width * ScreenCenterFactor, Screen.height * ScreenCenterFactor);
            ...
            if (!InventoryOpen && !DialogueOpen && !ImageOpen)
            {
                ...
                if (scroll.y < 0)
                {
                    Cam.fieldOfView = Mathf.Clamp(Cam.fieldOfView + FOVStep, MinFOV, MaxFOV);
                    FOV = Cam.fieldOfView;
                }
                else if (scroll.y > 0)
                {
    ...
    private void UpdateStabilityTimerText()
    {
        if (StabilityCoroutineRunning)
        {
            ...
            if (StabilityCountdown < (StabilityDuration - StabilityWarningOffset))
            {
    ...
    public void OpenCutscene(string videoName)
    {
        ...
        VideoPlayerLoaderConstruction.VideoPath = Path.Combine(Application.streamingAssetsPath, nameOnly + ".mp4");
    ...
    public void Spawn(string action, GameObject hotspot = null)
    {
        ...
        if (hotspot != null)
        {
            ...
            if (SpawnedHotspotActions.Contains(hotspotKey))
            {
                ...
                if (AudioManagerConstruction.Instance != null)
                    AudioManagerConstruction.Instance.PlaySFX(clip);
                return;
            }
        ...
        foreach (GameObject element in SpawnedGameObjects)
        {
            ...
            if (y_position >= (Y0Position - SpawnHeightBuffer))
            {
        ...
        if (CameraMovement != null)
        {
            CameraMovement.SetNewRotation(PreviewRotationX, PreviewRotationY);
        ...
        {
            Debug.LogWarning("Construction: CameraMovementConstruction reference is missing. Preview alignment and rotation adjustments will be skipped.", this);
        }
    ...
    private void RotateObjectLeft()
    {
        ...
        CurrentSpawnedObject.transform.Rotate(0f, -ObjectRotationStep, 0f, Space.World);
    }
    ...
    private void RotateObjectForward()
    {
        ...
        CurrentSpawnedObject.transform.Rotate(ObjectRotationStep, 0f, 0f, Space.World);
    }
    ...
    private void ProcessPointerSelection(Vector2 startPosition, Vector2 endPosition, string source)
    {
        ...
        if (DeltaMouse.magnitude >= MouseDragThreshold)
            return;
